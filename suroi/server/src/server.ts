    ...
    Bun.serve({
        ...
        routes: {
            ...
            "/api/addBots": async req => {
                ...
                const bots = botManager.addBot(Math.min(count, 50)); // 최대 50개

                return Response.json({
                    success: true,
                    message: `${bots.length}개의 AI 봇이 추가되었습니다.`,
                    addedCount: bots.length,
                    totalBots: botManager.getBotCount()
                }, corsHeaders);
            },
            // AI 봇 제거 API
            "/api/removeBot": async req => {
                const searchParams = getSearchParams(req);
                const id = parseInt(searchParams.get("id") || "0");

                const botManager = AIBotManager.getInstance();
                const removed = botManager.removeBot(id);

                return Response.json({
                    success: removed,
                    message: removed ? "봇이 제거되었습니다." : "봇을 찾을 수 없습니다.",
                    totalBots: botManager.getBotCount()
                }, corsHeaders);
            },
            // 모든 봇 제거 API
            ...
            "/api/bots": async req => {
                const botManager = AIBotManager.getInstance();
                const bots = botManager.getBots().map(b => ({
                    id: b.player.id,
                    name: b.player.name,
